...
const FriendButton: React.FC<Props> = ({ profileId, isSameUser }) => {
  ...
  if (status === "friends") {
    return (
      <button
        className={`${baseClass} bg-[#241c12] hover:bg-red-600`}
        disabled={loading}
  ...
  if (status === "requested") {
    return (
      <button className={`${baseClass} bg-[#241c12] cursor-default`} disabled>
        Request sent
      </button>
    );
  }
  ...
  if (status === "incoming") {
    return (
      <div className="flex gap-2">
        <button
          className={`${baseClass} bg-amber-600 hover:bg-amber-700`}
          disabled={loading}
