...
const LiveBets: React.FC<GameHistory> = ({ gameState }) => {
  const { t } = useTranslation();
    ...
    return (
        <div className="flex flex-col px-8 py-4 bg-[#1c1813] rounded file:h-min lg:w-[1140px] w-full">
            ...
            <div className="flex flex-col">
                ...
                <div className="flex items-center w-full justify-between p-2 bg-[#181410] rounded">
                    <span className="font-bold text-sm">{t("games.user")}</span>
                    <div className="hidden w-1/4 justify-between md:flex">
                        {
                            ["Bet", "Payout", "Profit"].map((item) => (
                                <span key={item} className="font-bold text-sm">{t("games." + item.toLowerCase())}</span>
                            ))
