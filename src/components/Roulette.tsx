...
const Roulette: React.FC<Roulette> = ({ items, openedItem, spin, className, direction = "horizontal" }) => {
  ...
  return (
    <div className={`flex  ${direction == "vertical" ? "max-h-[1100px]" : "max-w-[1100px]"} overflow-hidden ${className}`}>
      ...
      <div className={`absolute  ${direction == "vertical" ? "top-0 inset-x-0" : "left-0 inset-y-0 bg-gradient-to-r"} w-24 h-full  from-[#0e0e12] via-transparent`} />
      <div className={`absolute  ${direction == "vertical" ? "bottom-0 inset-x-0 " : "right-0 inset-y-0 bg-gradient-to-l"} w-24 h-full  from-[#0e0e12] via-transparent`} />
    </div>
  );
};
